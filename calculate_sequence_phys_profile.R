...
#'
#' @examples
dynchars<-function(seq, interval, tss, strand=1) {
  ...
  # split_seq <- split(seq(nchar(seq)),unlist(strsplit(seq,'')))
  # countA <- length(split_seq[['A']])
  # countT <- length(split_seq[['T']])
  # countG <- length(split_seq[['G']])
  # countC <- length(split_seq[['C']])
  ...
  if (strand==1) {
    M<-cbind(countA, countT, countG, countC)/interval
